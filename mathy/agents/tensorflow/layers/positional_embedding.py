# TF 2.0 Keras Positional Embedding using sin/cosine
# Copyright (c) 2018 Zhao HG
# License: MIT
# Source: https://github.com/CyberZHG/keras-pos-embd
...
class TrigPosEmbedding(keras.layers.Layer):
    ...
    def __init__(self, mode=MODE_ADD, output_dim=None, **kwargs):
        ...
        if mode in [self.MODE_EXPAND, self.MODE_CONCAT]:
            ...
            if output_dim % 2 != 0:
                raise NotImplementedError(
                    "It does not make sense to use an odd output dimension: %d"
                    % output_dim
                )
        self.mode = mode
    ...
    def get_config(self):
        config = {"mode": self.mode, "output_dim": self.output_dim}
    ...
    def call(self, inputs, mask=None):
        ...
        if self.mode == self.MODE_ADD:
            ...
            pos_input = K.tile(
                K.expand_dims(K.arange(0, seq_len), axis=0), [batch_size, 1]
            )
        ...
        odd_embd = K.cos(
            K.dot(
                ...
                K.expand_dims(
                    ...
                    / K.pow(
                        10000.0,
                        K.cast((odds - 1), K.floatx()) / K.cast(output_dim, K.floatx()),
                    ),
                    0,
                ),
            )
