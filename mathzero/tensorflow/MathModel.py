...
class MathModel():
    def __init__(self, game, args):
        ...
        with self.graph.as_default(): 
            ...
            s_fc1 = Dropout(Relu(BatchNormalization(Dense(h_conv4_flat, 64), axis=1, training=self.isTraining)), rate=self.dropout) # batch_size x 1024
            s_fc2 = Dropout(Relu(BatchNormalization(Dense(s_fc1, 32), axis=1, training=self.isTraining)), rate=self.dropout)         # batch_size x 512
