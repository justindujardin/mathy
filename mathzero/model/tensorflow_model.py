class MathModel:
    def __init__(self, game, args):
        ...
        with self.graph.as_default():
            ...
            h_conv1 = Relu(
                BatchNormalization(
                    self.conv2d(x_image, args.num_channels, "same"),
                    axis=3,
            ...
            h_conv2 = Relu(
                BatchNormalization(
                    self.conv2d(h_conv1, args.num_channels, "same"),
                    axis=3,
                    training=self.isTraining,
                )
            )  # batch_size  x board_x x board_y x num_channels
            h_conv3 = Relu(
                BatchNormalization(
                    self.conv2d(h_conv2, args.num_channels, "valid"),
                    axis=3,
                    training=self.isTraining,
                )
